...
class Generate
{
    ...
    public function run($data, $option = 'all')
    {
        ...
        // 载入默认配置
        $defaultConfigFile = \think\facade\Env::get('app_path') . 'admin' . DIRECTORY_SEPARATOR . 'extra' . DIRECTORY_SEPARATOR . 'generate.php';
        if (file_exists($defaultConfigFile)) {
            $data = array_merge(include $defaultConfigFile, $data);
        }
        // 检查目录是否可写
        $pathCheck = \think\facade\Env::get('app_path') . $data['module'] . DIRECTORY_SEPARATOR;
        ...
        if (isset($data['model']) && $data['model']) {
            ...
            $pathCheck = \think\facade\Env::get('app_path') . $module . DIRECTORY_SEPARATOR;
            if (!self::checkWritable($pathCheck)) {
                throw new Exception("目录没有权限不可写，请执行一下命令修改权限：<br>chmod -R 755 " . realpath($pathCheck), 403);
            }
        }
        ...
        // 删除刚刚生成的文件
        if (isset($data['delete_file']) && $data['delete_file']) {
            $pathView = \think\facade\Env::get('app_path') . $this->module . DIRECTORY_SEPARATOR . "view" . DIRECTORY_SEPARATOR . $this->dir . $this->nameLower . DIRECTORY_SEPARATOR;
            $fileName = \think\facade\Env::get('app_path') . "%MODULE%" . DIRECTORY_SEPARATOR . "%NAME%" . DIRECTORY_SEPARATOR . $this->dir . $this->name . ".php";
        ...
        if (isset($data['validate']) && $data['validate']) {
            $module = $this->readConfig($this->module, 'app', 'validate_path', Config::get('app.validate_path'));
            $dir_list[] = $module . DIRECTORY_SEPARATOR . "validate" . DIRECTORY_SEPARATOR . $this->dir;
        }
        if ($this->dir) {
        ...
        if ($action != 'buildDir') {
            // 文件路径
            $pathView = \think\facade\Env::get('app_path') . $this->module . DIRECTORY_SEPARATOR . "view" . DIRECTORY_SEPARATOR . $this->dir . $this->nameLower . DIRECTORY_SEPARATOR;
            $pathTemplate = \think\facade\Env::get('app_path') . 'admin' . DIRECTORY_SEPARATOR . "view" . DIRECTORY_SEPARATOR . "generate" . DIRECTORY_SEPARATOR . "template" . DIRECTORY_SEPARATOR;
            $fileName = \think\facade\Env::get('app_path') . "%MODULE%" . DIRECTORY_SEPARATOR . "%NAME%" . DIRECTORY_SEPARATOR . $this->dir . $this->name . ".php";
            $code = $this->parseCode();
            // 执行方法
    ...
    {
        try {
            $path = $path ? $path : \think\facade\Env::get('app_path') . 'admin' . DIRECTORY_SEPARATOR;
    ...
    private function delModel($pathView, $phpFile = '')
    {
        ...
        $file = \think\facade\Env::get('app_path') . $module . DIRECTORY_SEPARATOR . "model" . DIRECTORY_SEPARATOR . $name . ".php";
    ...
    private function buildDir($dir_list)
    {
        foreach ($dir_list as $dir) {
            $path = \think\facade\Env::get('app_path') . $dir;
            if (!is_dir($path)) {
                // 创建目录
    ...
    private function parseCode()
    {
        ...
        $sortable = false;
        // 生成 form.html 文件的代码
        $search = ['<form class="mb-20" method="get" action="{:\\\\think\\\\facade\\\\Url::build($Request.action)}">'];
        // 生成 th.html 文件的代码
                    ...
                    switch ($form['search_type']) {
                        ...
                        default:
                            ...
                            $filter .= tab(2) . 'if ($this->request->param("' . $form['name'] . '")) {' . "\n"
                                . tab(3) . '$map[\'' . $form['name'] . '\'] = [\'' . $form['name'] . '\',"like", "%" . $this->request->param("' . $form['name'] . '") . "%"];' . "\n"
                                . tab(2) . '}' . "\n";
                ...
                if (!in_array($form['name'], ['id', 'isdelete', 'create_time', 'update_time'])) {
                    ...
                    switch ($form['type']) {
                        ...
                        case "select":
                            ...
                            break;
                        case "textarea":
                            // 默认生成的textarea加入了输入字符长度实时统计，H-ui.admin官方的textarealength方法有问题，请使用 测试1.0 框架修改后的源码，也可拷贝 H-ui.js 里相应的方法
    ...
    {
        // 可能的配置文件路径
        $fileConfig = \think\facade\Env::get('app_path') . $module . '/config.php';
        $fileExtra = \think\facade\Env::get('app_path') . $module . 'extra/' . $scope . '.php';
        $config = [];
        // 加载配置
