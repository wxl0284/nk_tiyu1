...
trait Controller
{
    ...
    public function add()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            // 验证
            if (class_exists($validateClass = \think\facade\App::parseClass(config('app.validate_path'), 'validate', $controller))) {
            ...
            // 写入数据
            if (
                class_exists($modelClass = \think\facade\App::parseClass(config('app.model_path'), 'model', $this->parseCamelCase($controller)))
                || class_exists($modelClass = \think\facade\App::parseClass(config('app.model_path'), 'model', $controller))
            ) {
    ...
    {
        $id = $this->request->param('id');
        $vo = $this->getModel($this->request->controller())->find($id);

        $this->view->assign("vo", $vo);

        if(method_exists ($this,'extraArr')){
            $this->view->assign("extra", $this->extraArr());
        }
    ...
    public function changeFlag(){
        $id = input('id');
        $flag = input('flag');
        $model = Db::name($this->request->controller());
        $res = $model->find($id);
        if($res[$flag]){
