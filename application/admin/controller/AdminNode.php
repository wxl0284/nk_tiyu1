...
//    protected static $blacklist = ['recyclebin', 'delete', 'deleteforever', 'clear', 'recycle'];
            ...
            try {
                ...
                if ($this->request->param('type') == 'group') {
                    ...
                    $list = Db::name('admin_node')
                        ...
                        ->order('sort asc,id asc')
                        ->select();
                    $list = array_merge($list,Db::name('admin_node')->where("isdelete=0 AND level>2")->order('sort asc,id asc')->select());
                    // 重新组装节点
    ...
    /*public function index()
    {
        ...
        //分组信息
        $list_group = \think\facade\App::model('AdminGroup')->getList();
    ...
    public function sort()
    {
        ...
        \think\facade\App::model('AdminNode')->save($data, ['id' => $data['id']]);
    ...
    {
        //分组
        $group_list = \think\facade\App::model('AdminGroup')->getList();
    ...
    public function load()
    {
        if ($this->request->isPost()) {
            ...
            $error = \think\facade\App::model('AdminNode', 'logic')->insertLoad($node_template, $node_detect, $data);
