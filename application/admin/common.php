...
function getCidName($value,$name=''){
    $res = Db::name('category')->where('id',$value)->field('pid,name')->find();
    if($res['pid']){
        return getCidName($res['pid'],$res['name'].'->'.$name);

    }else{
        return rtrim($res['name'].'->'.$name,'->');
    }
 ...
 * @return string
 */
function show_check_status($status, $id, $field = 'id', $controller = '')
{
    $controller === '' && $controller = Request::instance()->controller();
    switch ($status) {
        // 恢复
        case 1 :
            $ret = '<a id="suc'.$id.'" href="javascript:;" onclick="ajax_req(\'' . Url::build($controller . '/checkSuccess', [$field => $id]) . '\',change_check_status('.$id.',\'resume\',this))" class="label label-success radius" title="通过">通过</a>';
            break;
        // 禁用
        case 2 :
            $ret = '<a id="err'.$id.'" href="javascript:;" onclick="ajax_req(\'' . Url::build($controller . '/checkError', [$field => $id]) . '\',change_check_status('.$id.',\'forbid\',this))" class="label label-warning radius" title="驳回">驳回</a>';
            break;
    }
 ...
 * @return string
 */
function get_check_status($status, $imageShow = true)
{
    switch ($status) {
        case 0 :
            $showText = '待审核';
            $showImg = '<i class="Hui-iconfont c-warning check-status" title="待审核">&#xe631;</i>';
            break;
        case 1 :
            $showText = '通过';
            $showImg = '<i class="Hui-iconfont c-success check-status" title="通过">&#xe615;</i>';

            break;
        case 2 :
        default :
            $showText = '驳回';
            $showImg = '<i class="Hui-iconfont c-error check-status" title="驳回">&#xe6dd;</i>';
...
function get_model($modelName)
{
    if (false !== strpos($modelName, '\\')) {
        ...
        try {
            $db = \think\facade\App::model($modelName);
