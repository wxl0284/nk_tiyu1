...
class Controller
{
    ...
    {
        $map = [];
        $table_info = $model->getFieldsType();

        $param = array_merge($this->request->param(), $param);
        foreach ($param as $key => $val) {
            if ($val !== "" && array_key_exists($key, $table_info)) {
                $map[$key] = $val;
            }
        }
    ...
    {
        $module = config('app.model_path');
        if (!$controller) {
            $controller = $this->request->controller();
        }
        if (
            class_exists($modelClass = App::parseClass($module, 'model', $this->parseCamelCase($controller)))
            || class_exists($modelClass = App::parseClass($module, 'model', $controller))
        ) {
    ...
    /**
     * 格式化类名，将 /. 转为 \\
     * 已废弃，请使用\think\facade\App::parseClass()
     *
     * @param string $name
     *
     * @return mixed
    ...
    protected function datalist($model, $map, $field = null, $sortBy = '', $asc = false, $return = false, $paginate = true)
    {
        ...
        if ($paginate) {
            ...
            if ($return) {
                ...
                $this->view->assign("page", $list->render().'   <style>
