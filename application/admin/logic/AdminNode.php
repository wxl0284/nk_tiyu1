...
class AdminNode
{
    ...
    {
        if (ADMIN) {
            $nodes = Db::name("AdminNode")->where("status=1 AND isdelete=0 AND group_id > 0")->field("id,pid,name,group_id,title,type")->order('sort asc')->select();
        } else {
    ...
    public function insertLoad($node_template, $node_detect, $data)
    {
        $error = [];
        $insert_all = [];
        $validate = new \app\common\validate\AdminNode();
